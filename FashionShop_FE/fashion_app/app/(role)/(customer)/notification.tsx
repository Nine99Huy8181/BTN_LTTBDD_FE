...
const openDeepLink = (deepLink: string, router: any) => {
  try {
    if (deepLink.startsWith('app://order/')) {
      // app://order/27 -> navigate to profile order-detail inside same (customer) group
      const orderId = deepLink.replace(/^app:\/\/order\//, '');
      router.push((`(profile)/order-detail/${orderId}` as unknown) as any);
    } else if (deepLink.startsWith('app://')) {
      const path = deepLink.replace(/^app:\/\//, '');
      // Keep previous behavior for other app:// paths — attempt to navigate
