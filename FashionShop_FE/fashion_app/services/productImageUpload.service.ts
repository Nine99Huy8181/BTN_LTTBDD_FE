...
/**
 * Upload ảnh product lên Cloudinary
...
export async function uploadProductImage(uri: string): Promise<string> {
  try {
    ...
    console.log('Product image compressed');

    // 2. Tạo tên file với prefix product
    ...
    if (!uploadRes.ok) {
      ...
      console.log('Product image upload failed:', uploadRes.status, errorData);
    ...
    if (data.secure_url) {
      console.log('Product image upload success:', data.secure_url);
      return data.secure_url;
    } else {
      throw new Error('Upload failed: No secure_url returned');
    }
  } catch (err) {
    console.log('Product image upload error:', err);
    throw err;
  }
}
